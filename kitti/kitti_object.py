...
class kitti_object(object):
    ...
    def __init__(self, root_dir, split='training', pseudo=False, refinement=False):
        ...
        if split == 'training':
            ...
            exit(-1)
        self.num_samples = 2e5
        self.image_dir = os.path.join(self.split_dir, 'image_2')
        self.calib_dir = os.path.join(self.split_dir, 'calib')
        self.lidar_dir = os.path.join(self.split_dir, 'velodyne')
        self.label_dir = os.path.join(self.split_dir, 'label_2')
        if pseudo:
            self.label_dir = os.path.join(root_dir, 'data/')
        if refinement:
            self.label_dir = os.path.join(self.split_dir, 'label_2_pseudo') 
