...
def compute_box_2d(center, dimension, heading, P):
    R = roty(heading)
    ...
    corners_2d = project_to_image(np.transpose(corners_3d), P).T
    
    return np.array([min(corners_2d[0]), min(corners_2d[1]), max(corners_2d[0]), max(corners_2d[1])])
