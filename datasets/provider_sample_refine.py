    ...
    def __init__(self, npoints, split,
        ...
        if not overwritten_data_path:
            if not from_rgb_detection:
                if split == 'val' or split == 'pseudo':
                    split += '_det'

                if car_only:
                    overwritten_data_path = os.path.join(root_data, 'refine_caronly_%s.pickle' % (split))
                elif people_only:
                    overwritten_data_path = os.path.join(root_data, 'refine_pedcyc_%s.pickle' % (split))
                else:
                    overwritten_data_path = os.path.join(root_data, 'refine_carpedcyc_%s.pickle' % (split))
            else:
                if car_only:
                    overwritten_data_path = os.path.join(root_data,
                                                         'refine_caronly_%s_rgb_detection_refine.pickle' % (split))
                elif people_only:
                    overwritten_data_path = os.path.join(root_data, 'refine_pedcyc_%s_rgb_detection_refine.pickle' % (split))
                else:
                    overwritten_data_path = os.path.join(
                        root_data, 'refine_carpedcyc_%s_rgb_detection_refine.pickle' % (split))
