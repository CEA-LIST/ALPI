...
from datasets.data_utils import rotate_pc_along_y, project_image_to_rect, compute_box_3d, compute_box_2d, extract_pc_in_box3d, roty
...
class ProviderDataset(Dataset):
    ...
    def __init__(self, npoints, split,
                 ...
                 extend_from_det=False, step=0, ratio=100):
        ...
        if not overwritten_data_path:
            if not from_rgb_detection:
                if car_only:
                    if split == 'train':
                        if step > 0:
                            overwritten_data_path = os.path.join(root_data, 'frustum_caronly_' + 'train_step_0' + str(step) + '_ratio_0' + str(int(ratio / 10)) + add + '.pickle')
                        else:
                            overwritten_data_path = os.path.join(root_data, 'frustum_caronly_' + 'train_step_0' + str(step) + '.pickle')
                    else:
                        overwritten_data_path = os.path.join(root_data, 'frustum_caronly_%s.pickle' % (split))
                elif people_only:
                    if split == 'train':
                        if step > 0:
                            overwritten_data_path = os.path.join(root_data, 'frustum_pedonly_' + 'train_step_0' + str(step) + '_ratio_0' + str(int(ratio / 10)) + add + '.pickle')
                        else:
                            overwritten_data_path = os.path.join(root_data, 'frustum_pedonly_' + 'train_step_0' + str(step) + '.pickle')
                    else:
                        overwritten_data_path = os.path.join(root_data, 'frustum_pedonly_%s.pickle' % (split))
                else:
                    if split == 'train':
                        if step > 0:
                            overwritten_data_path = os.path.join(root_data, 'frustum_carpedcyc_' + 'train_step_0' + str(step) + '_ratio_0' + str(int(ratio / 10)) + add + '.pickle')
                        else:
                            overwritten_data_path = os.path.join(root_data, 'frustum_carpedcyc_' + 'train_step_0' + str(step) + '.pickle')
                    else:
                ...
                if car_only:
                    ...
                    overwritten_data_path = os.path.join(root_data, 'frustum_pedonly_%s_rgb_detection.pickle' % (split))
                else:
    ...
    def __getitem__(self, index):
        ...
        if self.from_rgb_detection:
            ...
            data_inputs = {
                ...
                # '2d_bb_gt' : torch.FloatTensor(self.gt_box2d_list[index]),
                'calib' : torch.FloatTensor(self.calib_list[index]['P2'].reshape((3, 4))),
        ...
        data_inputs = {
            ...
            'center_ref3': torch.FloatTensor(ref3).transpose(1, 0),
            'center_ref4': torch.FloatTensor(ref4).transpose(1, 0),
            'bb_2d_gt' : torch.FloatTensor(compute_box_2d(box3d_center, box3d_size, heading_angle, self.calib_list[index]['P2'].reshape((3, 4)))),
            'calib' : torch.FloatTensor(self.calib_list[index]['P2'].reshape((3, 4))),
    ...
    def generate_labels(self, center, dimension, angle, ref_xyz, P):
        box_corner1 = compute_box_3d(center, np.array(dimension) * 0.5, angle)
...
if __name__ == '__main__':
    ...
    cfg.DATA.RTC = True
    dataset = ProviderDataset(1024, split='val', random_flip=True, one_hot=True, random_shift=True, step=0, ratio=100)
